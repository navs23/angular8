...
import { EventsAppModule } from 'src/app/events-app.module';
...
export class EventService {
  getEvents(): Subject<IEvent[]>{
    ...
    setTimeout(() => {
        subject.next(EVENTS);subject.complete();}, 1 * 1000);
    return subject
}
SaveEvent(event:IEvent):void{
  console.log('saving event\n%s ',JSON.stringify(event))

  event.id = this.getMaxEventId() + 1;    
  EVENTS.push(event);
  console.log('event saved successfully \n %s ',this.getMaxEventId())
...
getMaxEventId ():number{
  ...
  return Math.max.apply(Math,EVENTS.map(evt=> {return evt.id;}))
}
...
EVENTS.forEach(event=>{
  ...
  matchingSessions.map((session:any)=>{
    ...
    return results.concat(matchingSessions);
})
